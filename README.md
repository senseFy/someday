...
The `CI` workflow runs the hermetic Gradle `check` contract for every pull
request and every push to `main`. It does not contact developer localhost
services. The separate `Android` workflow builds a debug APK when a tag is
...
does not access signing credentials, notarize artifacts, or contact an app
store.
...
make ios-archive IOS_TEAM_ID=YOUR_TEAM_ID
make ios-export IOS_TEAM_ID=YOUR_TEAM_ID
make ios-upload-archive IOS_TEAM_ID=YOUR_TEAM_ID IOS_ARCHIVE_PATH=iosApp/build/Someday-Release.xcarchive
...
IOS_BUMP_VERSION=yes|no
IOS_ARCHIVE_PATH=/path/to/Someday-Release.xcarchive
IOS_EXPORT_PATH=/path/to/export
APP_STORE_CONNECT_APP_ID=<asc-app-id>
APP_STORE_CONNECT_API_KEY_PATH=/path/AuthKey_XXXXXX.p8
APP_STORE_CONNECT_API_KEY_ID=<key-id>
APP_STORE_CONNECT_API_ISSUER_ID=<issuer-id>
# ASC_* and EXPO_ASC_* aliases are also accepted
IOS_PROVISIONING_PROFILE_SPECIFIER="<App Store profile name>"
IOS_UPLOAD=yes
```

Upload auth defaults to App Store Connect API key mode. For Xcode Accounts-based upload, pass `IOS_UPLOAD_AUTH_MODE=accounts`. Ensure the local Apple Distribution certificate and App Store provisioning profile for `saien.someday` are available before packaging.
