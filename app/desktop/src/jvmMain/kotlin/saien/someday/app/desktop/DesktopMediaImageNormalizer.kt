...
internal object DesktopMediaImageNormalizer : MediaImageNormalizer {
    ...
    private fun decodeOrientedBounded(
        ...
        Codec.makeFromData(data).use codecUse@{ codec ->
            ...
            bitmap.use {
                ...
                if (oriented.image.width == target.width && oriented.image.height == target.height) {
                    oriented
                } else {
                    oriented.replaceWithScaled(target.width, target.height)
                }
            }
        }
    }
    ...
    private fun ManagedImage.replaceWithScaled(width: Int, height: Int): ManagedImage {
        ...
        val scaled = Surface.makeRasterN32Premul(width, height).use { surface ->
    ...
    private fun ManagedImage.encode(format: EncodedImageFormat, jpegQuality: Int): ByteArray? =
        image.encodeToData(format, jpegQuality)?.use { it.bytes }
