...
class IosWebDavCredentialStore : WebDavCredentialStore {
    override fun load(): String? = loadText(account)

    override fun save(secret: String) {
        require(secret.isNotBlank()) { "WebDAV credential must not be blank." }
        saveText(account, secret)
    }
    ...
    override fun saveForAuthority(credentials: WebDavAuthorityCredentials) {
        saveText(authorityAccount(credentials.authorityBindingId), credentials.encodeForSecureStorage())
    }
        ...
        withBaseQuery(accountName) { query ->
            ...
            memScoped {
                ...
                val dataRef = result.value?.reinterpretData()
                    ?: error("iOS Keychain returned an unexpected WebDAV credential payload.")
                try {
    ...
    private companion object {
        const val service = "saien.someday.webdav.credential"
        ...
        fun keychainError(action: String, status: Int): String =
            "iOS Keychain could not $action the WebDAV credential (status $status)."
    }
}
