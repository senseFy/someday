...
/**
 * Chooses the only valid local data surface while serialized against first
 * activation. Before key-bound V2 state exists, import/export uses the same
 * local product rows that genesis will checkpoint. Once any key-bound epoch
 * exists (including a prepared checkpoint), routing must stay on V2 and fail
 * closed rather than creating an invisible second local history.
 */
class AuthorityCoordinatedLocalDataTransferV2(
    private val localRepository: SqlDelightLocalDataRepository,
    private val authorityMutationCoordinator: WorkspaceAuthorityMutationCoordinator,
    private val v2Transfer: WorkspaceLocalDataTransferV2,
) {
