...
class TransportTlsFailureV2Test {
    @Test
    fun certificateAndHostnameFailuresBlockCredentialsTokensAndMutationsBeforeHttpDispatch() {
            ...
            val untrustedFailure = assertFails {
                WebDavClient(
                    WebDavConfiguration(
                        ...
                        "certificate-user-sentinel",
                        "certificate-secret-sentinel",
                        "/someday/",
                    ),
                    JdkWebDavTransport(),
                ).uploadRawAppendOnly("someday/untrusted-object", "untrusted-mutation-sentinel".encodeToByteArray())
            }
            ...
            val hostnameFailure = assertFails {
                WebDavClient(
                    WebDavConfiguration(
                        ...
                        "hostname-user-sentinel",
                        "hostname-secret-sentinel",
                        "/someday/",
                    ),
                    JdkWebDavTransport(trustedClient),
                ).uploadRawAppendOnly("someday/hostname-object", "hostname-mutation-sentinel".encodeToByteArray())
            }
            assertTrue(hostnameFailure.hasTlsHandshakeCause())
            assertEquals(0, fixture.httpDispatches.get())

            val tokenFailure = assertFails {
                JdkSelfHostedSyncTransport(trustedClient).v2Capabilities(
                    "https://127.0.0.1:${fixture.port}",
                    "bearer-token-sentinel",
                )
            }
            assertTrue(tokenFailure.hasTlsHandshakeCause())
            assertEquals(0, fixture.httpDispatches.get())
        } finally {
            fixture.close()
        }
