...
- `someday.systemV2ReleaseEnabled` and `someday.systemV2DevelopmentEnabled`
  default false. Canonical shipping entrypoints must explicitly set release
  true and development false; local debug must opt in. Never reintroduce a
  silent `systemV2ActivationEnabled = true` default or ship a release package
  whose generated BuildConfig leaves first-epoch activation disabled.
